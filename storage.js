...
promise.then(response => {
    ...
    oursSelect.then((ours) => {
        ...
        for (i = 0; i < localStorage.length; i++) {
            ...
            picBearPanier.setAttribute('src', localStorage.getItem(ours.name));
...
for (i = 0; i < localStorage.length; i++) {
    ...
    let picBearPanier = document.createElement('img');
    picBearPanier.classList.add('pictedPanier');
    picBearPanier.setAttribute('src', localStorage.getItem(Article, Object.entries(img)));
    // création du contenant pour afficher les infos //
    let oursInfoPanier = document.createElement('div');
    oursInfoPanier.classList.add('infoPanier');
    // Nom du produit //
...
btnClear.classList.add('viderPanier');
btnClear.setAttribute('type', 'submit');
btnClear.setAttribute('value', 'Vider le panier');
